...
class AssociadoLoginController extends BaseController
{
  function login(Request $request) 
  {
    ...
    $ass = Associado::where('ASS_EMAIL', $email)->where('ASS_ATIVADO', 'S')->first();
    if(isset($ass)) {
      if (Hash::check($password, $ass->ASS_SENHA)) {
        session(['tipo' => 'ASSOCIADO', 'ass' => $email, 'associado_password'=>$password, 'associado_id'=>$ass->ASS_ID]);
  ...
  function registrar() 
  {
    ...
    return view('registrar', ['sindicatos' => $sindicatos]);
  }
