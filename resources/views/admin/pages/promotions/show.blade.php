        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="col-lg-12" style="display:flex; justify-content:space-between">
                    @if(!$promotion->is_aprooved)
                        <form action="{{route('promotions.aproove',$promotion->id)}}" od="post" class="form-inline">
                            @csrf
                            <button type="submit" class="btn btn-success"><i class="fa fa-check"></i> Aprovar: <strong>{{ $promotion->title }}</strong></button>
                        </form>
                    @endif
                    ...
                    <form action="{{route('promotions.destroy',$promotion->id)}}" method="post" class="form-inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger"><i class="fa fa-trash"></i> Remover:
                            <strong>{{ $promotion->title }}</strong></button>
                    </form>
                </div>
            </div>
        </div>
